...
export default async function PracticeTestsPage() {
  ...
  // Create 20 mock practice tests
  const practiceTests = Array.from({ length: 20 }, (_, i) => {
    const testNum = i + 1;
    const isLocked = !isPremium && testNum > 10;
    return {
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-body-sm text-mute max-w-xl leading-relaxed">
              Test-format practice sessions to review module integration and timing. Short, focused evaluations to keep your preparation on track.
            </p>
          </div>
            ...
            {isPremium ? (
              ...
              <div className="flex flex-col gap-1 items-start md:items-end justify-center px-4 py-3 rounded-lg bg-warning-soft/30 border border-warning/15 shrink-0">
                ...
                <span className="text-display-sm font-bold text-ink">
                  10 <span className="text-xs font-normal text-mute">/ 20 Unlocked</span>
                </span>
              </div>
            )}
          </div>
        ...
        {practiceTests.map((test) => {
          return (
            ...
            >
              ...
              {!test.isLocked && (
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-gradient-preview-start to-gradient-preview-end opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              )}
