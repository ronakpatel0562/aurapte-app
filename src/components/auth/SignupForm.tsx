...
import { User, Mail, Phone, Lock, Eye, EyeOff, AlertCircle, CheckCircle, ArrowRight } from "lucide-react";
...
function SignupFormInner() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result && result.error) {
      ...
      setSuccessMessage(result.message || "Account created successfully! Please check your email to verify.");
      setLoading(false);
    }
  };
    ...
    <div className="w-full bg-white/90 backdrop-blur-md p-8 rounded-2xl border border-zinc-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.03)] animate-fade-in transition-all">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-zinc-500 mt-2">
          Start practicing for your PTE exam today
        </p>
      </div>
      ...
      {successMessage && (
        <div className="mb-6 p-3.5 text-xs bg-green-50 text-green-700 border border-green-100 rounded-xl flex items-start gap-2.5 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label
            ...
            className="block text-xs font-semibold text-zinc-700 uppercase tracking-wider mb-2"
          >
            Phone Number <span className="text-zinc-400 font-normal lowercase">(Optional)</span>
          </label>
          <div className="relative flex items-center">
      ...
      <button
        ...
        className="w-full h-11 border border-zinc-200 bg-white hover:bg-zinc-50 hover:border-zinc-300 text-sm font-medium rounded-xl text-zinc-700 transition active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2.5 cursor-pointer shadow-sm"
      >
        {googleLoading ? (
          <span className="w-4.5 h-4.5 border-2 border-zinc-400 border-t-transparent rounded-full animate-spin" />
        ) : (
          <>
...
export default function SignupForm() {
  return (
    <Suspense fallback={
      <div className="w-full max-w-[400px] p-8 bg-white/90 rounded-2xl border border-zinc-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.03)] animate-pulse h-[500px] flex flex-col justify-between">
